...
install_python_binding_dependencies()
{
    ...
    -H pip install -vU pygccxml https://bitbucket.org/ompl/pyplusplus/get/1.8.0.tar.gz
    # install castxml
    if [[ $ubuntu_version > 1410 ]]; then
...
install_app_dependencies()
{
    ...
    if [[ $ubuntu_version > 1410 ]]; then
        apt-get -y install python-pyqt5.qtopengl
    else
        apt-get -y install python-qt4-dev python-qt4-gl
    fi
    apt-get -y install freeglut3-dev libassimp-dev python-opengl python-flask python-celery
    # install additional python dependencies via pip
    pip install -vU PyOpenGL-accelerate
    # install libccd
    if [[ $ubuntu_version > 1410 ]]; then
